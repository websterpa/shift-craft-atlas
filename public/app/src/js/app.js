...
class ShiftCraftApp {
    ...
    init() {
        ...
        if (!localStorage.getItem('seed_v3_staff_restore')) {
            this.addTestStaff(8);
            localStorage.setItem('seed_v3_staff_restore', 'true');
        }
    ...
    async initData() {
        try {
            ...
            const [staff, shifts, config] = await Promise.all([
                ...
                this.absenceStore.init() // Ensure absence types are seeded
            ]);
    ...
    async saveToStorage() {
        try {
            await Promise.all([
                ...
                this.repo.saveStaff(this.staff)
            ]);
        } catch (error) {
    ...
    setupEventListeners() {
        ...
        if (logoutBtn) {
            logoutBtn.onclick = async () => {
                const result = await this.confirm({
                    title: 'Log Out',
                    body: 'Are you sure you want to log out and return to the access gate?',
                    icon: 'log-out',
                    iconColor: 'salmon' // Or var(--status-error)
                });

                if (result && result.confirmed) {
                    // 1. Clear Gate Session
                    sessionStorage.removeItem('invite_ok');

                    // 2. Sign out of Supabase (if authenticated)
                    try {
